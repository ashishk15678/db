...
impl Catalog {
    ...
    pub fn create_table(
        ...
        let db_name = data.current_database.clone()
            .ok_or("No database selected")?;
        
        let db = data.databases.get_mut(&db_name)
    ...
    pub fn get_table(&self, name: &str) -> Result<TableSchema, String> {
        ...
        let db_name = data.current_database.as_ref()
            .ok_or("No database selected")?;
        
        let db = data.databases.get(db_name)
            .ok_or(format!("Database '{}' not found", db_name))?;
...
mod tests {
    ...
    fn test_catalog_create_duplicate_table_error() {
        ...
        catalog.create_table("dup_table", columns.clone(), false).unwrap();
    ...
    fn test_catalog_create_table_if_not_exists() {
        ...
        catalog.create_table("exists_table", columns.clone(), false).unwrap();
    ...
    fn test_catalog_list_tables() {
        ...
        catalog.create_table("table1", columns.clone(), false).unwrap();
    ...
    fn test_data_type_to_string() {
        ...
        assert_eq!(data_type_to_string(&DataType::Varchar(Some(255))), "VARCHAR(255)");
